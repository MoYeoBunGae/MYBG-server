...
public class S3ImageService {
    ...
    private void validateImageFile(MultipartFile multipartFile) {
        ...
        if (contentType == null || !contentType.startsWith("image/")) {
            throw new ApplicationException(
                    ApplicationExceptionType.S3_FILE_FORMAT_EXCEPTION, "지원하지 않는 파일 형식입니다.");
        }

        if (originalFilename == null || !hasAllowedExtension(originalFilename)) {
            throw new ApplicationException(
                    ApplicationExceptionType.S3_FILE_FORMAT_EXCEPTION,
                    String.format("파일이름이 없거나 허용되지 않는 확장자입니다.(%s)", originalFilename));
        }
    ...
    private String getFileExtension(String fileName) {
        if (fileName == null) {
            throw new ApplicationException(
                    ApplicationExceptionType.S3_FILE_FORMAT_EXCEPTION, "파일 이름이 없습니다.");
        }
        int dotIndex = fileName.lastIndexOf(".");
        if (dotIndex == -1) {
            throw new ApplicationException(
                    ApplicationExceptionType.S3_FILE_FORMAT_EXCEPTION, "파일 확장자가 없습니다.");
        }
