...
@Repository
public class CustomBungaeRecruitDateOptionRepositoryImpl
        implements CustomBungaeRecruitDateOptionRepository {
    ...
    @Override
    public List<BungaeDateVoteInfoDto> findVoteInfoByDates(
            Long bungaeId, List<LocalDate> dateOptions, Long groupMemberId) {
        QBungaeRecruitDateOption bungaeRecruitDateOption =
                QBungaeRecruitDateOption.bungaeRecruitDateOption;
        QBungaeDateVote bungaeDateVote = QBungaeDateVote.bungaeDateVote;

        // 총 득표수 서브쿼리
        SubQueryExpression<Integer> totalVotesSubquery =
                JPAExpressions.select(bungaeDateVote.count().intValue())
                        .from(bungaeDateVote)
                        .where(bungaeDateVote.dateOption.id.eq(bungaeRecruitDateOption.id));

        BooleanExpression userVotedCondition =
        ...
        return queryFactory
                .select(
                        new QBungaeDateVoteInfoDto(
                                bungaeRecruitDateOption, totalVotesSubquery, userVotedCondition))
