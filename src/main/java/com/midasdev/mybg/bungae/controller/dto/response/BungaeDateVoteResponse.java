...
public record BungaeDateVoteResponse(
        ...
        @Schema(description = "번개 날짜가 확정되었는지 여부. ", example = "true") boolean isDateFixed,
        @Schema(
                        description =
                                "확정된 날짜에 추가로 참여 가능한지 여부. <br/>"
        ...
        @Schema(
                        ...
                        nullable = true)
                LocalDate fixedDate,
        @Schema(description = "번개의 현재 상태. ", example = "DATE_VOTING") BungaeStatus bungaeStatus,
        @Schema(
                        description =
                                "투표가 가능한 번개에 대해 투표에 실패한 날짜 목록.<br/>"
