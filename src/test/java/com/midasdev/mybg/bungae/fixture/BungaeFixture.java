...
public class BungaeFixture {
    ...
    public static Bungae createWithDateVoting(
            Group group, GroupMember host, int minAttendees, int maxAttendees) {
    ...
    public static Bungae createWithRecruiting(
            Group group, GroupMember host, LocalDate date, int minAttendees, int maxAttendees) {
        return bungaeBaseBuilder(group, host, minAttendees, maxAttendees)
                .status(BungaeStatus.RECRUITING)
                .bungaeDateTime(new BungaeDateTime(date))
                .build();
    ...
    public static Bungae createWithRecruitingClosed(
            Group group, GroupMember host, LocalDate date, int minAttendees, int maxAttendees) {
        return bungaeBaseBuilder(group, host, minAttendees, maxAttendees)
                .status(BungaeStatus.RECRUITING_CLOSED)
                .bungaeDateTime(new BungaeDateTime(date))
    ...
    private static Bungae.BungaeBuilder bungaeBaseBuilder(
            Group group, GroupMember host, int minAttendees, int maxAttendees) {
    ...
    public static Bungae createWithBungaeDateTime(
            Group group, GroupMember host, BungaeDateTime bungaeDateTime) {
        return Bungae.builder()
                .name("테스트 번개")
                .description("테스트용 번개 설명")
                .minAttendees(2)
